...
| Disponibilidade | Parcial | Body limit, timeout, pool limit, concorrência, rate limits global/login/registro/mutação | Testes de rate limit `429`, readiness `503`, concorrência saturada `503`; config tipada | Teste controlado de saturação de DB e slow request no proxy privado ainda pendente |

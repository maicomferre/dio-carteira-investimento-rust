...
| Disponibilidade | Parcial | Body limit, timeout, pool limit, concorrência, rate limits global/login/registro/mutação; contrato privado de borda | Testes de rate limit `429`, readiness `503`, concorrência saturada `503`, pool saturado `503` e `EDGE_PRIVATE_VALIDATION.md` | Execução real de slow request no proxy privado ainda pendente |
| Banco e integridade | Implementado | PostgreSQL, SQLx parametrizado, migrations versionadas, credenciais dev separadas para migration/runtime, constraints de ownership | Migrations, `DATABASE_MIGRATION_URL`, `container/postgres/init/001-dev-roles.sql` e testes SQLi/IDOR/ownership | Backup/restore e roles reais de produção pertencem à operação privada |
...
| Supply chain | Implementado | Lockfiles, `cargo audit`, `npm audit`, Trivy filesystem, SBOM CycloneDX ignorado pelo Git | `scripts/audit-supply-chain.sh`, `specs/SECURITY_AUDIT.md`, `LICENSE` | Projeto licenciado sob MIT; SBOM permanece fora do Git |

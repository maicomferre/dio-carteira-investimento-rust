...
| Disponibilidade | Parcial | Body limit, timeout, pool limit, concorrência, rate limits global/login/registro/mutação | Testes de rate limit `429`, readiness `503`, concorrência saturada `503` e pool saturado `503`; config tipada | Teste de slow request no proxy privado ainda pendente |
